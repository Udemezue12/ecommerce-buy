...
from .models import OrderItem, Order, Address, Payment, Coupon, Product, Refund, UserProfile
            ...
            if form.is_valid():
                ...
                if use_default_shipping:
                    ...
                    if address.exists():
                        ...
                        messages.info(
                            self.request, "No Default Shipping Address Avaliable")
                        return redirect('core:checkout')
            else:
                print("Enter New Shipping Address")
                shipping_address1 = form.cleaned_data.get('shipping_address1')
                shipping_address2 = form.cleaned_data.get('shipping_address2')
                shipping_zip = form.cleaned_data.get('shipping_zip')
                shipping_country = form.cleaned_data.get('shipping_country')
                if form.is_valid([shipping_address1, shipping_address2, shipping_country, shipping_zip]):

                    shipping_address_qs = Address.objects.filter(user=self.request.user,
                                                                 apartment_address=shipping_address1,
                                                                 country=shipping_country,
                                                                 zip=shipping_zip,
                                                                 street_address=shipping_address2)

                    shipping_address_qs.save()

                    order.address = shipping_address_qs
                    order.save()
                    ...
                    if set_default_shipping:
                        shipping_address_qs.default = True
                        shipping_address_qs.save()
